...
function toggleSkillsPhysics() {
    const btn = document.getElementById('physics-btn');
    if (skillsGraph.physicsEnabled) {
        skillsGraph.simulation.stop();
        btn.textContent = 'Resume';
        btn.style.background = '#f39c12';
    } else {
        skillsGraph.simulation.restart();
        btn.textContent = 'Pause';
        btn.style.background = '#667eea';
    }
    skillsGraph.physicsEnabled = !skillsGraph.physicsEnabled;
}

function fitSkillsToScreen() {
    const bounds = skillsGraph.svg.select(".graph-group").node().getBBox();
    const padding = 40;
    const scale = Math.min(
        skillsGraph.width / (bounds.width + padding),
        skillsGraph.height / (bounds.height + padding)
    ) * 0.8;

    const translateX = (skillsGraph.width - bounds.width * scale) / 2 - bounds.x * scale;
    const translateY = (skillsGraph.height - bounds.height * scale) / 2 - bounds.y * scale;

    skillsGraph.svg.transition()
        .duration(750)
        .call(skillsGraph.svg.__zoom.transform, d3.zoomIdentity
